...
let tags = [];
const inputTagContainer = document.querySelector('#input_tag');
...
inputTagContainer.classList.add('input_tags_container');
tagsContainer.classList.add('tag_container');
...
inputTagContainer.addEventListener('click', (e) => {
    ...
    if (
        e.target.id === 'input_tag' ||
        e.target.classList.contains('tag_container')
    ) {
        inputTag.focus();
    }
});
